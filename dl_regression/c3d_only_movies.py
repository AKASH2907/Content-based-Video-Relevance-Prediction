...
datapath = 'movies/track_2_movies/feature/'
...
# List of all the C3D features present in Movies
def make_list():
    train_X_c3d_features = []

    for i in range(10826):
...
def ground_truth():
    ...
    relevance_file = pd.read_csv('relevance_train_movies.csv', header=None)
    relevance_file.fillna(-1, inplace=True)

    # Initialize ground truth matrix of shape= (4500, 10826)
    gd = np.zeros([relevance_file.shape[0], 10826])
...
# Training model
def train_model():
    train_x = read_csv_file('relevance_train_movies.csv')[0]
    model = model_c3d()
    ...
    for epoch in range(10):
        for i, j in enumerate(train_x):
            ...
            # Batch wise training
            val = model.train_on_batch([train_c3d[j]], gd[i].reshape(1, 10826))

            # Print number of epochs, iteration number, loss
            print(epoch, i, val)
            # Save model after 50 iterations
            if i % 50 == 0 and i % 100 != 0:
                model.save_weights('c3d_50_30fm.h5')
            # Save model after 100 iterations
            if i % 100 == 0:
                model.save_weights('c3d_100_30fm.h5')

        # Save model after 1 epoch
        model.save_weights('c3d_epoch_30fm.h5')
...
def predict():
    ...
    predict_model.load_weights('c3d_epoch_30fm.h5')

    # Read validation file
    val_x = read_csv_file('movies/track_2_movies/split/val.csv')[0]
    for i in val_x:
